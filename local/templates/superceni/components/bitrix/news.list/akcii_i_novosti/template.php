...
<div class="filter-block">
    <a href="?clear_cache=Y" class="filter<?if(empty($_GET['year'])){?> filter-active<?}?>">Все</a>
    <a href="?year=<?=date('Y');?>&clear_cache=Y" class="filter<?if($_GET['year'] == date('Y')){?> filter-active<?}?>"><?=date('Y');?></a>
    <a href="?year=<?=date('Y')-1;?>&clear_cache=Y" class="filter<?if($_GET['year'] == date('Y')-1){?> filter-active<?}?>"><?=date('Y')-1;?></a>
    <a href="?year=<?=date('Y')-2;?>&clear_cache=Y" class="filter<?if($_GET['year'] == date('Y')-2){?> filter-active<?}?>"><?=date('Y')-2;?></a>
</div>
<div class="row ms-0 me-0 mt-5" id="news_items">
                ...
                <div id="<?=$this->GetEditAreaId($arItem['ID']);?>" class="col-12 p-0 col-md-4 news-item <?=($key > 0 ) ? 'mt-3' : ''?>  mt-md-0">
                    <div class="col-12 col-md-11 mx-md-auto col-xxl-12 me-xxl-auto">
                        ...
                        <div class="news-item_date mt-2">
                            <span><?= explode(' ',$arItem_properties["DATE_CREATE"])[0];?></span>
                        </div>
